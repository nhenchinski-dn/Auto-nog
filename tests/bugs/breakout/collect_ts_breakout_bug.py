#!/usr/bin/env python3
"""Collect tech-support from wky1c7vd00008p2 for breakout-head URPF validation bug."""

import paramiko, time, re

DUT_IP = '100.64.8.59'
TS_NAME = 'breakout-bug'
...
scp_cmd = (
    ...
    f'/techsupport/{tar_filename if tar_filename else "ts_breakout-bug_*"} '
...
while shell.recv_ready():
    out += shell.recv(65535).decode("utf-8", errors="replace")
print(f"  SCP output tail: {clean(out)[-300:]}")
