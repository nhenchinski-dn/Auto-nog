#!/usr/bin/env python3
"""Test ACL binding with hierarchical config mode - step by step."""
...
# Attach ACL to interface - step by step (like apply_acl.py)
cmd('interfaces ge10-0/0/32', 2)
out = cmd('access-list ipv4 qa-ipv4 direction ingress', 5)
print(f'Bind ipv4 to ge10-0/0/32: {out[-300:]}', flush=True)
cmd('top', 2)

# Show the candidate config
print('\n=== CANDIDATE CONFIG ===', flush=True)
...
cmd('configure', 3)
cmd('interfaces ge10-0/0/32', 2)
cmd('no access-list ipv4 qa-ipv4', 3)
cmd('top', 2)
