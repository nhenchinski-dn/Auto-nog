#!/usr/bin/env python3
"""ACL MIB test with correct binding syntax: direction in/out."""
import paramiko, time, re, sys
sys.stdout.reconfigure(line_buffering=True)

host = 'XEC1E3VR00008'
user = 'dnroot'
pw = 'dnroot'

ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect(host, username=user, password=pw, timeout=15, look_for_keys=False, allow_agent=False)

chan = ssh.invoke_shell(width=400, height=1000)
time.sleep(5)
chan.recv(65535)

ANSI_RE = re.compile(r'\x1b\[[0-9;]*[a-zA-Z]|\x1b\].*?\x07|\x1b[()][AB012]|\x1b\[\?[0-9;]*[hlm]|\r')

def cmd(c, wait=5):
    chan.send(c + '\n')
    time.sleep(wait)
    out = b''
    while chan.recv_ready():
        out += chan.recv(65535)
    return ANSI_RE.sub('', out.decode(errors='replace')).strip()

# Cleanup any leftover
cmd('configure', 3)
cmd('rollback', 5)
cmd('end', 3)
out = cmd('', 2)
if 'uncommitted' in out.lower():
    cmd('no', 3)

# Configure everything cleanly
print('=== CONFIGURING ===', flush=True)
cmd('configure', 3)

# SNMP community
cmd('system snmp community qa-test vrf default', 3)
cmd('top', 2)

# IPv4 ACL
cmd('access-lists', 2)
cmd('ipv4 qa-ipv4', 2)
cmd('rule 10 allow', 2)
cmd('exit', 2)
cmd('rule 20 deny', 2)
cmd('exit', 2)
cmd('exit', 2)  # exit acl

# IPv6 ACL
cmd('ipv6 qa-ipv6', 2)
cmd('rule 10 allow', 2)
cmd('exit', 2)
cmd('exit', 2)  # exit acl

# Ethernet ACL
cmd('eth qa-eth', 2)
cmd('rule 10 allow', 2)
cmd('exit', 2)
cmd('exit', 2)  # exit acl

cmd('exit', 2)  # exit access-lists

# Bind ACLs with correct syntax: direction "in" not "ingress"
cmd('interfaces ge10-0/0/32 access-list ipv4 qa-ipv4 direction in', 5)
cmd('top', 2)
cmd('interfaces ge10-0/0/33 access-list ipv6 qa-ipv6 direction in', 5)
cmd('top', 2)
cmd('interfaces ge10-0/0/34 access-list eth qa-eth direction in', 5)
cmd('top', 2)

# Show candidate diff
print('\n=== CONFIG DIFF ===', flush=True)
print(cmd('show config diff | no-more', 15), flush=True)

# Commit
print('\n=== COMMIT ===', flush=True)
out = cmd('commit', 25)
has_error = bool(re.search(r'error|failed', out, re.IGNORECASE))
print(f'Commit: {"FAIL" if has_error else "OK"}', flush=True)
if has_error:
    print(out[-1000:], flush=True)
    cmd('rollback', 5)
cmd('end', 3)
out = cmd('', 2)
if 'uncommitted' in out.lower():
    cmd('no', 3)

if not has_error:
    # Verify committed config for interfaces
    print('\n=== COMMITTED ge10-0/0/32 ===', flush=True)
    print(cmd('show config interfaces ge10-0/0/32 | no-more', 10), flush=True)

    print('\n=== COMMITTED ge10-0/0/33 ===', flush=True)
    print(cmd('show config interfaces ge10-0/0/33 | no-more', 10), flush=True)

    print('\n=== COMMITTED ge10-0/0/34 ===', flush=True)
    print(cmd('show config interfaces ge10-0/0/34 | no-more', 10), flush=True)

    # CLI counters
    print('\n=== CLI COUNTERS ===', flush=True)
    print(cmd('show access-lists counters | no-more', 10), flush=True)

    print('\n=== CLI COUNTERS ge10-0/0/32 ===', flush=True)
    print(cmd('show access-lists counters ge10-0/0/32 | no-more', 10), flush=True)

    # Wait for SNMP refresh
    print('\n--- Waiting 100s for SNMP agent refresh ---', flush=True)
    time.sleep(100)

    # Walk ACL MIB
    print('\n=== MAPPING TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.1" | no-more', 30), flush=True)

    print('\n=== IPv4 COUNTER TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.2" | no-more', 30), flush=True)

    print('\n=== IPv6 COUNTER TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.3" | no-more', 30), flush=True)

    print('\n=== ETH COUNTER TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.4" | no-more', 30), flush=True)

    print('\n=== DEFAULT IPv4 TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.5" | no-more', 30), flush=True)

    print('\n=== DEFAULT IPv6 TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.6" | no-more', 30), flush=True)

    print('\n=== DEFAULT ETH TABLE ===', flush=True)
    print(cmd('run system snmp walk "1.3.6.1.4.1.49739.2.17.1.7" | no-more', 30), flush=True)

# CLEANUP
print('\n=== CLEANUP ===', flush=True)
cmd('configure', 3)
cmd('no interfaces ge10-0/0/32 access-list ipv4 qa-ipv4', 3)
cmd('top', 2)
cmd('no interfaces ge10-0/0/33 access-list ipv6 qa-ipv6', 3)
cmd('top', 2)
cmd('no interfaces ge10-0/0/34 access-list eth qa-eth', 3)
cmd('top', 2)
cmd('no access-lists ipv4 qa-ipv4', 3)
cmd('top', 2)
cmd('no access-lists ipv6 qa-ipv6', 3)
cmd('top', 2)
cmd('no access-lists eth qa-eth', 3)
cmd('top', 2)
cmd('no system snmp community qa-test', 3)
cmd('top', 2)
out = cmd('commit', 20)
print(f'Cleanup: {"FAIL" if "error" in out.lower() else "OK"}', flush=True)
if 'error' in out.lower():
    print(out[-500:], flush=True)
    cmd('rollback', 5)
cmd('end', 3)
out = cmd('', 2)
if 'uncommitted' in out.lower():
    cmd('no', 3)

ssh.close()
print('\nDONE', flush=True)
