...
def run_single_restart():
    ...
    while True:
        ...
        try:
            ...
            if is_target_ncp_up(output, TARGET_SERIAL):
                print("Target NCP is UP. Sending warm restart command...")
                break
    ...
    try:
        ...
        ssh_run_shell_with_confirm(
            client, "request system restart warm", confirm="yes", timeout=30
        )
    except Exception:
    ...
    while True:
        ...
        try:
            ...
            if is_up and (seen_down or time.time() - start_time >= POLL_INTERVAL):
                ...
                print(f"Warm restart time: {elapsed:.1f} seconds")
                return elapsed
            print("Device reachable, NCP not up yet")
            backoff = POLL_INTERVAL
        except Exception:
